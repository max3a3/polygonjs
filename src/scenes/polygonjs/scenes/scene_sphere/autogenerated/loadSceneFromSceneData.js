...
import { requiredImports_scene_sphere } from "./requiredImports";

const loadSceneFromSceneData_scene_sphere = async function (options) {
  const {
    domElement,
    sceneData,
    onProgress,
  ...
  if (runRegister) {
    ...
    for (const node of requiredImports_scene_sphere.nodes) {
      Poly.registerNode(node, undefined, { printWarnings });
    }
    for (const operation of requiredImports_scene_sphere.operations) {
      Poly.registerOperation(operation, { printWarnings });
    }
    for (const jsFunction of requiredImports_scene_sphere.jsFunctions) {
...
export { Poly, loadSceneFromSceneData_scene_sphere };
