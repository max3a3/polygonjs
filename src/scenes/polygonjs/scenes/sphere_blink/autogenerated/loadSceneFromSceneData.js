...
import { requiredImports_sphere_blink } from "./requiredImports";

const loadSceneFromSceneData_sphere_blink = async function (options) {
  const {
    domElement,
    sceneData,
    onProgress,
  ...
  if (runRegister) {
    ...
    for (const node of requiredImports_sphere_blink.nodes) {
      Poly.registerNode(node, undefined, { printWarnings });
    }
    for (const operation of requiredImports_sphere_blink.operations) {
      Poly.registerOperation(operation, { printWarnings });
    }
    for (const jsFunction of requiredImports_sphere_blink.jsFunctions) {
...
export { Poly, loadSceneFromSceneData_sphere_blink };
